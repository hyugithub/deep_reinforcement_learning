...
#source of confusion -- truncation should only be part of training
truncated_backprop_length = 15

#hidden state: no LSTM
state_size = 4
#label
num_classes = 2
#delay in output
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx in range(num_batches):
            ...
            if batch_idx%100 == 0:
                ...
                pred = np.swapaxes(np.vstack([np.argmax(y, axis=1) for y in _predictions_series]),0,1)
                print("Inaccuracy: %.2f " % (np.count_nonzero(batchY-pred)/np.size(batchY)))
